...
class course extends connect
{
    ...
    public function save()
    {
      ...
      {
         //To check course id is primary key or not
        	$f=0;
        	$sql="select * from course";
        	$r=mysqli_query($this->db_found, $sql);
        	while($db_field=mysqli_fetch_assoc($r))
         	{	 
             		if($db_field['course_id']==$_POST['t1'])
             		{
                		$f=1;
                		break;
             		}
        	...
        	if($f==1)
        	{
          	...
          	{
                    $sql="insert into course values('$_POST[t1]','$_POST[t2]','$_POST[t3]','$_POST[t4]')";
    ...
    public function update()
    {
        if($this->db_found==true)
       {
          $sql="update course set name='$_POST[t2]', desc1='$_POST[t3]',grade_id='$_POST[t4]' where course_id ='$_POST[t1]'";
        mysqli_query($this->db_found, $sql);
    ...
    public function delete()
    {
        if($this->db_found==true)
      {
        $sql="delete from course where course_id ='$_POST[t1]'";
        mysqli_query($this->db_found, $sql);
     ...
     public function psearch()
   {
     if($this->db_found==true)
     {
        $sql="select * from course where course_id='$_POST[t1]'";
        $r=mysqli_query($this->db_found, $sql);
       echo "<table border=1>
                   <tr>
                          <th>Course ID </th>
    ...
    public function spsearch()
    {
     ...
     {
        $colnm=$_POST["s"];
        $sql="select * from course where $colnm='$_POST[t1]'";
        $r=mysqli_query($this->db_found,$sql);
        echo"<table border=1>
        <tr>
            <th>Course ID</th>
        ...
        {
            echo"<tr><th>".$db_field['course_id']."</th>";
            echo"<tr><th>".$db_field['name']."</th>";
            echo"<tr><th>".$db_field['desc1']."</th>";
            echo "<th>".$db_field['grade_id']."</th>";
            echo"</tr>";
        }
        echo"</table>";
...
echo '<form name=f method=post action=course.php>
<body>
