   ...
   public function save()
    {
      ...
      {
        //To check parent id is primary key or not
        	$f=0;
        	$sql="select * from parent";
        	$r=mysqli_query($this->db_found, $sql);
        	while($db_field=mysqli_fetch_assoc($r))
         	{	 
             		if($db_field['parent_id']==$_POST['t1'])
             		{
                		$f=1;
                		break;
             		}
        	...
        	if($f==1)
        	{
          	...
          	{
                   $sql="insert into parent values('$_POST[t1]','$_POST[t2]','$_POST[t3]','$_POST[t4]','$_POST[t5]','$_POST[t6]','$_POST[t7]')";
    ...
    public function update()
    {
        if($this->db_found==true)
       {
          $sql="update parent set fname='$_POST[t2]', lname='$_POST[t3]',dob='$_POST[t4]',phone='$_POST[t5]',mobile='$_POST[t6]', status='$_POST[t7]' where parent_id ='$_POST[t1]'";
        mysqli_query($this->db_found, $sql);
    ...
    public function delete()
    {
        if($this->db_found==true)
      {
        $sql="delete from parent where parent_id ='$_POST[t1]'";
        mysqli_query($this->db_found, $sql);
  ...
  public function allsearch()
   {
     if($this->db_found==true)
     {
      ...
      while($db_field=mysqli_fetch_assoc($r))
      {
          echo "<tr><th>".$db_field['parent_id']."</th>";
   ...
   public function psearch()
   {
     if($this->db_found==true)
     {
        $sql="select * from parent where parent_id='$_POST[t1]'";
        $r=mysqli_query($this->db_found, $sql);
       echo "<table border=1>
                   <tr>
                          <th>Parent ID </th>
     ...
     public function spsearch()
    {
     ...
     {
        $colnm=$_POST["s"];
        $sql="select * from parent where $colnm='$_POST[t1]'";
        $r=mysqli_query($this->db_found,$sql);
        echo"<table border=1>
        <tr>
            <th>Parent ID</th>
