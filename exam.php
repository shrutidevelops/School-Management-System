...
class exam extends connect
{
   ...
   public function save()
      ...
      if($this->db_found==true)
      {
       //To check classroom is primary key or not
        	...
        	if($f==1)
        	{
          	...
          	{
                      $sql="insert into exam values('$_POST[t1]','$_POST[t2]','$_POST[t3]','$_POST[t4]')";
     ...
     public function update()
    {
        if($this->db_found==true)
       {
          $sql="update exam set exam_type_id='$_POST[t2]', name='$_POST[t3]',start_date='$_POST[t4]' where exam_id='$_POST[t1]'";
        mysqli_query($this->db_found, $sql);
    ...
    public function delete()
    {
        if($this->db_found==true)
      {
        $sql="delete from exam where exam_id ='$_POST[t1]'";
        mysqli_query($this->db_found, $sql);
    ...
    public function allsearch()
   {
     if($this->db_found==true)
     {
      ...
      while($db_field=mysqli_fetch_assoc($r))
      {
          ...
          echo "<th>".$db_field['exam_type_id']."</th>";
   ...
   public function psearch()
   {
     if($this->db_found==true)
     {
        $sql="select * from exam where exam_id='$_POST[t1]'";
        $r=mysqli_query($this->db_found, $sql);
       echo "<table border=1>
                   <tr>
                          <th>Exam ID </th>
    ...
    public function spsearch()
    {
     ...
     {
        $colnm=$_POST["s"];
        $sql="select * from exam where $colnm='$_POST[t1]'";
        $r=mysqli_query($this->db_found,$sql);
        echo"<table border=1>
        <tr>
            <th>Exam ID</th>
        ...
        {
            echo"<tr><th>".$db_field['exam_id']."</th>";
            echo"<tr><th>".$db_field['exam_type_id']."</th>";
            echo "<th>".$db_field['name']."</th>";
            echo "<th>".$db_field['start_date']."</th>";
            echo"</tr>";
        }
...
<tr>
    ...
    <td><select name=t2>';
          $db_found=mysqli_connect("localhost","root","1234","student");
			 $sql = 'select exam_type_id from exam_type'; 
			 $result = mysqli_query($db_found, $sql); 
			 while ($row = mysqli_fetch_assoc($result))
			 {
			    echo "<option value='" . $row['exam_type_id'] . "'>" . $row['exam_type_id'] . "</option>";
			 } 
echo '
</select>
</td>
