<?php
include "connect.php";
class exam_type extends connect
{
      ...
      if($this->db_found==true)
      {
        //To check classroom is primary key or not
        	...
        	while($db_field=mysqli_fetch_assoc($r))
         	{	 
             		if($db_field['exam_type_id']==$_POST['t1'])
             		{
                		$f=1;
                		break;
             		}
        	...
        	if($f==1)
        	{
          	...
          	{
                   $sql="insert into exam_type values('$_POST[t1]','$_POST[t2]','$_POST[t3]')";
   ...
   public function update()
     {
        if($this->db_found==true)
       {
          $sql="update exam_type set name='$_POST[t2]', desc1='$_POST[t3]' where exam_type_id='$_POST[t1]'";
        mysqli_query($this->db_found, $sql);
    ...
    public function delete()
    {
        if($this->db_found==true)
      {
        $sql="delete from exam_type where exam_type_id ='$_POST[t1]'";
        mysqli_query($this->db_found, $sql);
    ...
    public function allsearch()
   {
     ...
     public function psearch()
    {
     if($this->db_found==true)
     {
        $sql="select * from exam_type where exam_type_id='$_POST[t1]'";
        $r=mysqli_query($this->db_found, $sql);
       echo "<table border=1>
                   <tr>
                          <th>Exam type ID </th>
   ...
   public function spsearch()
    {
     ...
     {
        $colnm=$_POST["s"];
        $sql="select * from exam_type where $colnm='$_POST[t1]'";
        $r=mysqli_query($this->db_found,$sql);
        echo"<table border=1>
        <tr>
            <th>Exam type ID</th>
...
echo '<form name=f method=post action=exam_type.php>
<body>
<h2 align="center">EXAM TYPE</h2>
