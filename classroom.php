...
class classroom extends connect
{
      ...
      if($this->db_found==true)
      {
         //To check classroom is primary key or not
        	...
        	while($db_field=mysqli_fetch_assoc($r))
         	{	 
             		if($db_field['classroom_id']==$_POST['t1'])
             		{
                		$f=1;
                		break;
             		}
        	...
        	if($f==1)
        	{
          	...
          	{
             $sql="insert into classroom values('$_POST[t1]','$_POST[t2]','$_POST[t3]','$_POST[t4]','$_POST[t5]','$_POST[t6]','$_POST[t7]')";
    ...
    public function update()
     {
        if($this->db_found==true)
       {
          $sql="update classroom set year='$_POST[t2]', grade_id='$_POST[t3]', section='$_POST[t4]' , status='$_POST[t5]', remarks='$_POST[t6]', teacher_id='$_POST[t7]' where classroom_id='$_POST[t1]'";
        mysqli_query($this->db_found, $sql);
    ...
    public function delete()
    {
        if($this->db_found==true)
      {
        $sql="delete from classroom where classroom_id ='$_POST[t1]'";
        mysqli_query($this->db_found, $sql);
   ...
   public function allsearch()
   {
     if($this->db_found==true)
     {
      ...
      while($db_field=mysqli_fetch_assoc($r))
      {
          ...
          echo "<th>".$db_field['year']."</th>";
          echo "<th>".$db_field['grade_id']."</th>";
          echo "<th>".$db_field['section']."</th>";
          echo "<th>".$db_field['status']."</th>";
          echo "<th>".$db_field['remarks']."</th>";
          echo "<th>".$db_field['teacher_id']."</th>";
          echo "</tr>";
      }
       echo "</table>";
     }
    ...
    public function psearch()
   {
     if($this->db_found==true)
     {
        $sql="select * from classroom where classroom_id='$_POST[t1]'";
        $r=mysqli_query($this->db_found, $sql);
      echo "<table border=1>
                   <tr>
                          <th>Classroom ID</th>
    ...
    public function spsearch()
    {
     ...
     {
        $colnm=$_POST["s"];
        $sql="select * from classroom where $colnm='$_POST[t1]'";
        $r=mysqli_query($this->db_found,$sql);
        echo"<table border=1>
        <tr>
            <th>Classroom ID</th>
        ...
        {
            echo"<tr><th>".$db_field['classroom_id']."</th>";
            echo"<tr><th>".$db_field['year']."</th>";
            echo"<tr><th>".$db_field['grade_id']."</th>";
            echo"<tr><th>".$db_field['section']."</th>";
            echo"<tr><th>".$db_field['status']."</th>";
            echo"<tr><th>".$db_field['remarks']."</th>";
            echo "<th>".$db_field['teacher_id']."</th>";
            echo"</tr>";
        }
        echo"</table>";
...
echo '<form name=f method=post action=classroom.php>
<body>
<h2 align="center">CLASSROOM</h2>
...
<tr>
    ...
    <td><select name=t3>';
         $db_found=mysqli_connect("localhost","root","1234","student");
			 $sql = 'select grade_id from grade'; 
			 $result = mysqli_query($db_found, $sql); 
			 while ($row = mysqli_fetch_assoc($result))
			 {
			    echo "<option value='" . $row['grade_id'] . "'>" . $row['grade_id'] . "</option>";
			} 
...
<tr>
<th colspan=2><input type=submit value=New name=b0>
